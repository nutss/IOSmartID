...
var iconv = new Iconv('CP874', 'UTF-8//TRANSLIT//IGNORE'); //ADD
        ...
        return new Promise((resolve, reject) => {
            ...
            reader.transmit(cmd_select, 0xFF, protocol, (err, response) => {
                ...
                if (err) reject(err);
			        ...
			        reader.transmit(cmd_select, 0xFF, protocol, (err, response) => {});
				    reader.transmit(cmd_reset, 0xFF, protocol, (err, response) => {});    
				    reader.transmit(cmd_idcard1, 0xFF, protocol, (err, response) => {});    
				    reader.transmit(cmd_idcard2, 0xFF, protocol, (err, response) => {	                
						
						console.log(response.toString().substring(0,13));
						
						
						idcard = response.toString().substring(0,13).trim();
						smartcard.push(idcard);
						

					});

					reader.transmit(cmd_fullname1, 0xFF, protocol, (err, response) => {});     
				    reader.transmit(cmd_fullname2, 0xFF, protocol, (err, response) => {	                
				    ...
				    reader.transmit(cmd_address1, 0xFF, protocol, (err, response) => {});    
				    reader.transmit(cmd_address2, 0xFF, protocol, (err, response) => {

						response = iconv.convert(response).toString('utf8');
						address = response.toString().replace(/#/g, ' ').replace(/\0/g, '').trim();
						
						smartcard.push(address); 

					});
					
				    reader.transmit(cmd_expiredate1, 0xFF, protocol, (err, response) => {});    
				    reader.transmit(cmd_expiredate2, 0xFF, protocol, (err, response) => {	                
						
						

						response = iconv.convert(response).toString('utf8');
						expiredate = response.toString().replace(/#/g, ' ').replace(/\0/g, '').trim();
